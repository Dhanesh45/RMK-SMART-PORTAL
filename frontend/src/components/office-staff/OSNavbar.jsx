...
const OSNavbar = ({setActivePage}) => {
  const navigate=useNavigate();
  const osNavItems=["HOME","APPROVAL","GENERATE","HISTORY"];
  return (
      ...
      <nav className='h-[6.5vh] w-[96vw] bg-[#1E2E4F] text-white flex justify-between items-center rounded-[3.5vh] px-[1vw]'>
        ...
        <div className='flex items-center gap-[3vw]'>
         {osNavItems.map((item, index) => (
          ...
          <button onClick={()=>navigate("/OfficeStaffLogin")}className='bg-[#D9E1F1] text-[#1E2E4F] font-bold text-[2vh] px-[1.7vw] py-[0.7vh] rounded-[2.3vh] hover:bg-[#7be0cb] transition-colors duration-300'>
            LOGOUT
          </button>
        </div>
      </nav>
